...
def strassen_setup_s_sym(n,r,r2,m):
    #calculates how much padding is needed for static padding and call the cost equation
    d=1
    n,r,r2 = int(n),int(r),int(r2)
    for i in range(max(n,r,r2)):
        if 2**i >= max(n,r,r2):
            d=i
            break
    #Calls the symmetric cost equation for pebbling game Strassen algorithm with the padded rows and columns
    return strassen_sym(0,2**d,2**d, n,r,r2,m)

def strassen_setup_w_s_sym(n,r,r2,m):
    #calculates how much padding is needed for static padding and call the cost equation
    d=1
    n,r,r2 = int(n),int(r),int(r2)
    for i in range(max(n,r,r2)):
        if 2**i >= max(n,r,r2):
            d=i
            break
    #Calls the symmetric cost equation for winograd Strassen algorithm with the padded rows and columns
    return strassen_w_sym(0,2**d,2**d, n,r,r2,m)
        
def strassen_setup_s(n,r,n2,r2,m):
    #calculates how much padding is needed for static padding and call the cost equation
    d=1
    n,r,r2 = int(n),int(r),int(r2)
    for i in range(max(n,r,n2,r2)):
        if 2**i >= max(n,r,n2,r2):
            d=i
            break
    #Calls the non-symmetric cost equation for pebbling game Strassen algorithm with the padded rows and columns
    return strassen(0,2**d,2**d,2**d,2**d, n,r,n2,r2,m)

def strassen_setup_w_s(n,r,n2,r2,m):
    #calculates how much padding is needed for static padding and call the cost equation
    d=1
    n,n2,r,r2 = int(n),int(n2),int(r),int(r2)
    for i in range(max(n,r,n2,r2)):
        if 2**i >= max(n,r,n2,r2):
            d=i
            break
    #Calls the non-symmetric cost equation for winograd Strassen algorithm with the padded rows and columns
    return strassen_w(0,2**d,2**d,2**d,2**d, n,r,n2,r2,m)

def strassen_setup_static(n,r,m):
    #set up the partitioned multiplication with the pebble game Strassen algorithm with static padding
    if n<r:
        b=n
        a=0
    else:
    ...
    #return the operation cost equation for the multiplication of the submatrices with static padding and pebble game Strassen algorithm
    return a*strassen_setup_s_sym(r,r,r,m)+ a*(a+1)/2*strassen_setup_s(r,r,r,r,m)+a*strassen_setup_s(r,b,r,r,m)+strassen_setup_s_sym(r,b,b,m)

def strassen_setup_w_static(n,r,m):
    #set up the partitioned multiplication with the pebble game Strassen algorithm with static padding
    #If no sqaure submatrices can be made
    if n<r:
        b=n
        a=0
    #a is the amount of square submatrices and b is the remaining dimension
    else:
        b = n%r
        n=int(n)
        for i in range(n):
            if i*r+b==n:
                a=i
                break
    #return the operation cost equation for the multiplication of the submatrices with static padding and Winograd's Strassen algorithm
    return a*strassen_setup_w_s_sym(r,r,r,m)+ a*(a+1)/2*strassen_setup_w_s(r,r,r,r,m)+a*strassen_setup_w_s(r,b,r,r,m)+strassen_setup_w_s_sym(r,b,b,m)
...
def blocked_cholesky_part_static_w(d,n,r,m):
    #The operation cost equation of the blocked Cholesky factorization with matrix multiplication using partitioned Winograd's strassen algorithm with static padding
    ...
    if n-r<=0:
        ...
        const = (n-r)**2+(n-r)*(r*(r+1)/2*m+r*(r-1)/2)+strassen_setup_w_static(n-r,r,m)
        #set the minimum depth for the cutoff criteria to be included
        if d>=0:
            #the cutoff criteria
        ...
        return standard_cholesky(r,m)+const +blocked_cholesky_part_static_w(d+1,n-r,r,m)
    
def blocked_cholesky_part_static(d,n,r,m):
    #The operation cost equation of the blocked Cholesky factorization with matrix multiplication using partitioned pebble game strassen algorithm with static padding
    #if the matrix has no elements, no factorization can be made
    if n<=0:
        return 0
    #If no more iterations can be made, regular Cholesky factorization algorithm can be used
    if n-r<=0:
        ...
        const = (n-r)**2+(n-r)*(r*(r+1)/2*m+r*(r-1)/2)+strassen_setup_static(n-r,r,m)
        #set the minimum depth for the cutoff criteria to be included
        if d>=0:
            #the cutoff criteria
            if standard_cholesky(n,m)-standard_cholesky(n-r,m)-standard_cholesky(r,m)<=const:                       
                return standard_cholesky(n,m)
        #returning the operation cost of the steps and recursive calls of the cost equations
        return standard_cholesky(r,m)+const +blocked_cholesky_part_static(d+1,n-r,r,m)
...
def main_cholesky(n_max,steps,r,d):
    ...
    for i in range(steps):
        ...
        cho_p_s_w.append(blocked_cholesky_part_static_w(0,x*i,r,m))
        cho_p_s.append(blocked_cholesky_part_static(0,x*i,r,m))
    ...
    plt.plot(index,cho_p_s_w,"b-", label = "Partitioned Static Winograd")
    plt.plot(index,cho_p_s,"g-", label = "Partitioned Static Pebble game")

    
    #print difference between the standard algorithm and the blocked Cholesky factorization, negative procent means a speed-up

    print("rate:",(-standard_list[steps-1]+cho_p_s_w[steps-1])/standard_list[steps-1]*100)
    print("rate:",(-standard_list[steps-1]+cho_p_s[steps-1])/standard_list[steps-1]*100)
...
def main_strassen(n_max,steps,r,d):
    ...
    for i in range(steps):
        ...
        cho_p_s_w.append(strassen_setup_w_static(x*i,r,m))
        cho_p_s.append(strassen_setup_static(x*i,r,m))
