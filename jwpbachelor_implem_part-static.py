...
def strass_w(A,B,m,x1,y1,x2,y2):
    ...
    if A.ncols()-x1<=1 or A.nrows()-y1<=1 or B.ncols()-x2<=1 or B.nrows()-y2<=1:
        ...
        S2,T2 = S1-A11,B22-T1
        S4,T4 = A12-S2,B21-T2
        P1,P2,P3,P4,P5,P6,P7 = strass_w(A11,B11,m,x1_0,y1_0,x2_0,y2_0),strass_w(A12,B21,m,x1_r,y1_0,x2_0,y2_r),strass_w(S1,T1,m,x1_0,y1_r,x2_0,y2_0),strass_w(S2,T2,m,x1_0,y1_0,x2_0,y2_0),strass_w(S3,T3,m,x1_0,y1_0,x2_r,y2_0),strass_w(S4,B22,m,x1_0,y1_0,x2_r,y2_r),strass_w(A22,T4,m,x1_r,y1_r,x2_0,y2_0) 
        U1,U2 = P1+P2,P1+P4
        U3,U6 = U2+P5,U2+P3
...
def strass_sym_w(A,B,m,x1,y1,x2,y2):
    #Winograd Strassen algorithm with a symmetric result, m is the multiplication constant, x1,y1,x2,y2 represent the zero rows and columns of A and B respectively.
    #Check if more iterations can be made or if matrices are non-zero, if they are, use standard multiplication 
    if A.ncols()-x1<=1 or A.nrows()-y1<=1 or B.ncols()-x2<=1 or B.nrows()-y2<=1:
        if (x1<=1 and y1<=1) and (x2<=1 and y2<=1):
            return A*B
        return mult_sym(A,B,m,x1,y1,x2,y2)
    else:  
        #Set up new non zero dimensions
        x1_r,y1_r,x2_r,y2_r = min(x1,A.ncols()/2),min(y1,A.nrows()/2),min(x2,B.ncols()/2),min(y2,B.nrows()/2)
        x1_0,y1_0,x2_0,y2_0 = max(0,x1-A.ncols()/2),max(0,y1-A.nrows()/2),max(0,x2-B.ncols()/2),max(0,y2-B.nrows()/2)
        ...
        S2,T2 = S1-A11,B22-T1
        T4 = B21-T2
        P1,P2,P3,P4,P5,P7 = strass_sym_w(A11,B11,m,x1_0,y1_0,x2_0,y2_0),strass_sym_w(A12,B21,m,x1_r,y1_0,x2_0,y2_r),strass_sym_w(S1,T1,m,x1_0,y1_r,x2_0,y2_0),strass_w(S2,T2,m,x1_0,y1_0,x2_0,y2_0),strass_w(S3,T3,m,x1_0,y1_0,x2_r,y2_0),strass_w(A22,T4,m,x1_r,y1_r,x2_0,y2_0) 
        U1,U2 = P1+P2,P1+P4
        U3 = U2+P5
...
def strass(A,B,m,x1,y1,x2,y2):
    ...
    if A.ncols()-x1<=1 or A.nrows()-y1<=1 or B.ncols()-x2<=1 or B.nrows()-y2<=1:
        ...
        P1,P2,P3,P4,P5,P6,P7 = strass(A11,B11,m,x1_0,y1_0,x2_0,y2_0),strass(A12,B21,m,x1_r,y1_0,x2_0,y2_r),strass(A21,T1,m,x1_0,y1_r,x2_0,y2_0),strass(A22,B22,m,x1_0,y1_0,x2_0,y2_0),strass(S1,T2,m,x1_0,y1_r,x2_0,y2_r),strass(S2,T3,m,x1_0,y1_r,x2_0,y2_r),strass(S3,B12,m,x1_0,y1_0,x2_r,y2_0)
...
def strass_sym(A,B,m,x1,y1,x2,y2):
    ...
    if A.ncols()-x1<=1 or A.nrows()-y1<=1 or B.ncols()-x2<=1 or B.nrows()-y2<=1:
        ...
        P1,P2,P4,P5,P6,P7 = strass_sym(A11,B11,m,x1_0,y1_0,x2_0,y2_0),strass_sym(A12,B21,m,x1_r,y1_0,x2_0,y2_r),strass(A22,B22,m,x1_0,y1_0,x2_0,y2_0),strass(S1,T2,m,x1_0,y1_r,x2_0,y2_r),strass_sym(S2,T3,m,x1_0,y1_r,x2_0,y2_r),strass(S3,B12,m,x1_0,y1_0,x2_r,y2_0)
        temp = P5-P2-P4
        C11,C12,C22 = P1+P2,P5-P7,add_sym(P6,temp,0,0)
        C12 = C12-(P5+temp)
...
def partition_mult_s(A,m):
    ...
    for i in range(a):
        ...
        C = strassen_setup(listy[i].transpose(),listy[i],m)
        for k in range(r):
            for c in range(r):
        ...
        for j in range(i):
            C = strassen_setup(listy[i].transpose(),listy[j],m)
            C_t = C.transpose()
            for k in range(r):
                for c in range(r):
    ...
    for i in range(a+1):
        C = strassen_setup(listy[a].transpose(),listy[i],m)
        C_t = C.transpose()
...
def partition_mult_w_s(A,m):
    ...
    for i in range(a):
        #Add matrices on the diagonal
        C = strassen_setup_w(listy[i].transpose(),listy[i],m)
        for k in range(r):
            for c in range(r):
        ...
        #Add matrices below the diagonal
        for j in range(i):
            C = strassen_setup_w(listy[i].transpose(),listy[j],m)
            C_t = C.transpose()
            for k in range(r):
                for c in range(r):
                    res[c+i*r,k+j*r] = C[c,k]
    ...
    #Calculate the bottow row of submatrices
    for i in range(a+1):
        C = strassen_setup_w(listy[a].transpose(),listy[i],m)
        C_t = C.transpose()
...
def blocked_cholesky_w_static(A,r,m):
    ...
    if (m+1)/6*(n**3-r**3-(n-r)**3)+(m+1/2)*(n**2-r**2-(n-r)**2)<=(n-r+1)/(2*(n-r))*strassen_simple_w_sym(n-r,r,m):
        return standard_cholesky(A)
    ...
    if n>r:
        ...
        S = A_22- strassen_setup_w(R_12.transpose(),R_12,m)
...
def blocked_cholesky_static(A,r,m):
    #blocked cholesky algorithm for matrix A with stepsize r  
    n =A.nrows()
    a = mt.ceil((n-r)/r)
    if n-r<=0:
        return standard_cholesky(A)
    #Cutoff criteria, where both regular strassen or partitioned strassen can be used with strassen_simple calculated the approximate steps necessary for matrix multiplication
    if (m+1)/6*(n**3-r**3-(n-r)**3)+(m+1/2)*(n**2-r**2-(n-r)**2)<=(n-r+1)/(2*(n-r))*strassen_simple(n-r,r,m):
        return standard_cholesky(A)
    #If more steps of the algorithm can be made, preform the algorithm
    ...
    if n>r:
        ...
        S = A_22- strassen_setup(R_12.transpose(),R_12,m)
        #Combine the matrices
...
def blocked_cholesky_w_part_s(A,r,m):
    #blocked cholesky algorithm for matrix A with stepsize r  
    n =A.nrows()
    a = mt.ceil((n-r)/r)
    if n-r<=0:
    ...
    if n>r:
        ...
        S = A_22- partition_mult_w_s(R_12,m)
        #Combine the matrices
        L = merge_submatrix(R_11,R_12,blocked_cholesky_w_static(S,r,m))
    #If no more iterations can be preformed
    else:
...
def blocked_cholesky_part_s(A,r,m):
    #blocked cholesky algorithm for matrix A with stepsize r  
    n =A.nrows()
    a = mt.ceil((n-r)/r)
    if n-r<=0:
    ...
    if n>r:
        ...
        S = A_22- partition_mult_s(R_12,m)
        #Combine the matrices
        L = merge_submatrix(R_11,R_12,blocked_cholesky_static(S,r,m))
    #If no more iterations can be preformed
    else:
...
def experiment3(n_max,r,m):
    ...
    for i in range(n_max,2*n_max):
        ...
        for j in range(2):
            ...
            test = matrix*0
            start = time.time()
            test = blocked_cholesky_w_part_s(matrix,r,m)
            temp_blocked_p_w_s.append(time.time()-start)
            ...
            start = time.time()
            test = blocked_cholesky_part_s(matrix,r,m)
            temp_blocked_p_s.append(time.time()-start)
            if not is_same(test.transpose()*test,matrix):
                raise ValueError("klopt niet")
        ...
        listy_blk_p_w_s.append(sum(temp_blocked_p_w_s)/5)
        listy_blk_p_s.append(sum(temp_blocked_p_s)/5)
        listy_std.append(sum(temp_standard)/5)
        
    return listy_std,listy_blk_p_w_s,listy_blk_p_s
...
def main3(n,r,d):
    ...
    plt.plot(index, listy[1], 'g-', label = "Blocked static Winograd partition")
    plt.plot(index, listy[2], 'r-', label = "Blocked static pebble game partition")
    ...
    plt.show()
    x = (listy[7][n-1]-listy[0][n-2])/listy[0][n-1]*100
    y = (listy[8][n-1]-listy[0][n-2])/listy[0][n-1]*100
    tikzplotlib.save("test_cho_p_s_r="+str(r)+"_d=11_w="+str(x)+"s="+str(y)+".tex")
