...
def blocked_cholesky_static_w(d,n,r,m):
    ...
    if n-r<=0:
        ...
        const = (n-r)**2+(n-r)*(r*(r+1)/2*m+r*(r-1)/2)+strassen_setup_w_s_sym(n-r,r,r,m)
        #set the minimum depth for the cutoff criteria to be included
        if d>=0:
            #the cutoff criteria
        ...
        return standard_cholesky(r,m)+const +blocked_cholesky_static_w(d+1,n-r,r,m)

def blocked_cholesky_static(d,n,r,m):
    #The operation cost equation of the blocked Cholesky factorization with matrix multiplication using Winograd's strassen algorithm with static padding
    #if the matrix has no elements, no factorization can be made
    if n<=0:
        return 0
    ...
    if n-r<=0:
        ...
        const = (n-r)**2+(n-r)*(r*(r+1)/2*m+r*(r-1)/2)+strassen_setup_s_sym(n-r,r,r,m)
        #set the minimum depth for the cutoff criteria to be included
        if d>=0:
            #the cutoff criteria
            if standard_cholesky(n,m)-standard_cholesky(n-r,m)-standard_cholesky(r,m)<=const:
                ...
                return standard_cholesky(n,m)
        #returning the operation cost of the steps and recursive calls of the cost equations
        return standard_cholesky(r,m)+const +blocked_cholesky_static(d+1,n-r,r,m)
...
def main_cholesky(n_max,steps,r,d):
    ...
    for i in range(steps):
        ...
        cho_st_w.append(blocked_cholesky_static_w(0,x*i,r,m))
        cho_st.append(blocked_cholesky_static(0,x*i,r,m))
    ...
    plt.plot(index,cho_st_w,"b-", label = "Static Winograd")
    plt.plot(index,cho_st,"g-", label = "Static Pebble game")
  
    
    #print difference between the standard algorithm and the blocked Cholesky factorization, negative procent means a speed-up
    print("rate:",(-standard_list[steps-1]+cho_st_w[steps-1])/standard_list[steps-1]*100)
    print("rate:",(-standard_list[steps-1]+cho_st[steps-1])/standard_list[steps-1]*100)
...
def main_strassen(n_max,steps,r,d):
    ...
    for i in range(steps):
        ...
        cho_st_w.append(strassen_setup_w_s_sym(x*i,r,r,m))
        cho_st.append(strassen_setup_s_sym(x*i,r,r,m))
