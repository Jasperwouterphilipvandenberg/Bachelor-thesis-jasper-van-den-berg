...
def strassen_setup(n,r,m):
    #set up the partitioned multiplication with the pebble game Strassen algorithm with dynamic padding
    if n<r:
        b=n
        a=0
    else:
        ...
        for i in range(n):
            if i*r+b==n:
                a=i
                break
    #return the operation cost equation for the multiplication of the submatrices with dynamic padding and pebble game Strassen algorithm
    return a*strassen_sym(0,r,r,r,r,r,m)+ a*(a+1)/2*strassen(0,r,r,r,r,r,r,r,r,m)+a*strassen(0,r,r,r,r,r,b,r,r,m)+strassen_sym(0,r,r,b,b,b,m)

def strassen_setup_w(n,r,m):
    #set up the partitioned multiplication with the pebble game Strassen algorithm with dynamic padding
    if n<r:
        b=n
        a=0
    ...
    #return the operation cost equation for the multiplication of the submatrices with dynamic padding and Winograd's Strassen algorithm        
    return a*strassen_w_sym(0,r,r,r,r,r,m)+ a*(a+1)/2*strassen_w(0,r,r,r,r,r,r,r,r,m)+a*strassen_w(0,r,r,r,r,r,b,r,r,m)+strassen_w_sym(0,r,r,b,b,b,m)
...
def blocked_cholesky_part_w(d,n,r,m):
    #The operation cost equation of the blocked Cholesky factorization with matrix multiplication using partitioned Winograd's strassen algorithm with dynamic padding
    ...
    if n-r<=0:
        ...
        const = (n-r)**2+(n-r)*(r*(r+1)/2*m+r*(r-1)/2)+strassen_setup_w(n-r,r,m)
        #set the minimum depth for the cutoff criteria to be included
        if d>=0:
            #the cutoff criteria
        ...
        return standard_cholesky(r,m)+const +blocked_cholesky_part_w(d+1,n-r,r,m)
    
def blocked_cholesky_part(d,n,r,m):
    #The operation cost equation of the blocked Cholesky factorization with matrix multiplication using partitioned pebble game strassen algorithm with dynamic padding
    #if the matrix has no elements, no factorization can be made
    if n<=0:
        return 0
    #If no more iterations can be made, regular Cholesky factorization algorithm can be used
    if n-r<=0:
        ...
        const = (n-r)**2+(n-r)*(r*(r+1)/2*m+r*(r-1)/2)+strassen_setup(n-r,r,m)
        #set the minimum depth for the cutoff criteria to be included
        if d>=0:
            #the cutoff criteria
            if standard_cholesky(n,m)-standard_cholesky(n-r,m)-standard_cholesky(r,m)<=const:       
                return standard_cholesky(n,m)
        #returning the operation cost of the steps and recursive calls of the cost equations
        return standard_cholesky(r,m)+const +blocked_cholesky_part(d+1,n-r,r,m)
...
def main_cholesky(n_max,steps,r,d):
    ...
    for i in range(steps):
        ...
        cho_p_d_w.append(blocked_cholesky_part_w(0,x*i,r,m))
        cho_p_d.append(blocked_cholesky_part(0,x*i,r,m))
    ...
    plt.plot(index,cho_p_d_w,"b-", label = "Partitioned Dynamic Winograd")
    plt.plot(index,cho_p_d,"g-", label = "Partitioned Dynamic Pebble game")
    
    #print difference between the standard algorithm and the blocked Cholesky factorization, negative procent means a speed-up
    print("rate:",(-standard_list[steps-1]+cho_p_d_w[steps-1])/standard_list[steps-1]*100)
    print("rate:",(-standard_list[steps-1]+cho_p_d[steps-1])/standard_list[steps-1]*100)
...
def main_strassen(n_max,steps,r,d):
    ...
    for i in range(steps):
        ...
        cho_p_d_w.append(strassen_setup_w(x*i,r,m))
        cho_p_d.append(strassen_setup(x*i,r,m))
