...
def blocked_cholesky_w_dynamic(A,r,m):
    ...
    if n>r:
        ...
        S = A_22- dynamic_sym_w(R_12.transpose(),R_12,m,0,0,0,0)
...
def blocked_cholesky_dynamic(A,r,m):
    #blocked cholesky algorithm for matrix A with stepsize r  
    n =A.nrows()
    a = mt.ceil((n-r)/r)
    if n-r<=0:
        return standard_cholesky(A)
    #Cutoff criteria, with strassen_simple calculated the approximate steps necessary for matrix multiplication
    ...
    if n>r:
        ...
        S = A_22- dynamic_sym(R_12.transpose(),R_12,m,0,0,0,0)
        #Combine the matrices
...
def experiment(n_max,r,m):
    ...
    for i in range(n_max,2*n_max):
        ...
        for j in range(2):
            ...
            #Blocked Cholesky factorization with dynamic padding Winograd Strassen algorithm
            test = matrix*0
            start = time.time()
            test = blocked_cholesky_w_dynamic(matrix,r,m)
            temp_blocked_d_w.append(time.time()-start)
            if not is_same(test.transpose()*test,matrix):
                raise ValueError("klopt niet")
            #Blocked Cholesky factorization with dynamic padding pebble game Strassen algorithm
            test = matrix*0
            start = time.time()
            test = blocked_cholesky_dynamic(matrix,r,m)
            temp_blocked_d.append(time.time()-start)
            if not is_same(test.transpose()*test,matrix):
                raise ValueError("klopt niet")
        ...
        #Take the average of the timings for the results

        listy_blk_d_w.append(sum(temp_blocked_d_w)/5)
        listy_blk_d.append(sum(temp_blocked_d)/5)

        listy_std.append(sum(temp_standard)/5)
        
    return listy_std,listy_blk_d_w,listy_blk_d
...
def main(n,r,d):
    ...
    plt.plot(index, listy[1], 'g-', label = "Blocked Winograd dynamic")
    plt.plot(index, listy[2], 'r-', label = "Blocked pebble game dynamic")
    ...
    plt.show()
    x = (listy[3][n-1]-listy[0][n-2])/listy[0][n-1]*100
    y = (listy[4][n-1]-listy[0][n-2])/listy[0][n-1]*100
    tikzplotlib.save("test_cho_d_r="+str(r)+"_d=11_w="+str(x)+"s="+str(y)+".tex")
